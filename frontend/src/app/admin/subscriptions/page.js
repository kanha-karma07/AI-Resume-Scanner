...
export default function SubscriptionsList() {
  ...
  const fetchList = async (currentPage = 1, currentSearch = search, currentPlan = plan) => {
    try {
      setLoading(true);
      const res = await getSubscriptions({ page: currentPage, search: currentSearch, plan: currentPlan });
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">Manage payment history and active plans</p>
        </div>
                ...
                subscriptions.map((sub) => (
                  <tr key={sub.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-medium text-slate-800">{sub.user_name || "N/A"}</div>
                      <div className="text-xs text-slate-400">{sub.user_email}</div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="capitalize text-slate-600">{sub.user_role}</span>
                    </td>
                    <td className="px-6 py-4 font-medium text-slate-800">{sub.plan}</td>
                    <td className="px-6 py-4 font-medium text-slate-800">${sub.amount}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-slate-600">{sub.payment_type}</td>
                  </tr>
                ))
              )}
            </tbody>

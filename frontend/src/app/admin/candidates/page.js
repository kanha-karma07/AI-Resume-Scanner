...
export default function CandidatesList() {
  ...
  const fetchList = async (currentPage = 1, currentSearch = search, currentPlan = plan) => {
    try {
      setLoading(true);
      const res = await getCandidates({ page: currentPage, search: currentSearch, plan: currentPlan });
  ...
  }, [page, plan]); // Fetch on page or plan change
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">Manage registered candidates</p>
        </div>
              ...
              {loading ? (
                ...
                candidates.map((c) => (
                  <tr key={c.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 font-medium text-slate-800">{c.user.full_name || "N/A"}</td>
                    <td className="px-6 py-4 text-slate-600">{c.user.email}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                        c.user.is_premium ? "bg-purple-100 text-purple-700" : "bg-slate-100 text-slate-700"
                      }`}>
                        {c.user.is_premium ? "Premium" : "Freemium"}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-slate-600">{c.resume_count}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                        c.user.subscription_status === "ACTIVE" ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-700"
                      }`}>
                        {c.user.subscription_status}
                      </span>
                    </td>
